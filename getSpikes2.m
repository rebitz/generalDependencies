function [meanResponse,stdResponse,meanTrace,steTrace,trialData,FF,A] = getSpikes2(trials,evtOI,sigOI,pre,post)
%
...
%    6. spike-count variance / mean spike count (Fano Factor)
%    7. is the whole trial matrix A
%
...
meanResponse = mean(full(mean(A,2)));

if length(trials) > 1
%     stdResponse = mean(full(std(A,1))./sqrt(length(trials)-1));
    stdResponse = std(full(mean(A,2))); % changed 10/18/14, was wrong
    steTrace = full(std(A,1)./sqrt(length(trials)-1));
    
    FF = var(sum(A,2)./scaleby)./mean(sum(A,2)./scaleby);
else
    %disp('WARNING:')
    %disp('Too few trials (<5) for a meaningful variance estimate.')
    %disp('Returning NaNs.')
    stdResponse = NaN;
    steTrace = NaN;
    FF = NaN;
