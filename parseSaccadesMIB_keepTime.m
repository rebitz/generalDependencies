...
peakthresh = 150; %100 % must cross this to be a peak
saccthresh = 30; %20 % soft implementation - range of saccade starts
...
% try,        
    ...
    if trials(i).correct == 1 && sum(~isnan(trials(i).eye(:,1)))>10
        ...
        fixIdx2 = and(trials(i).eye(:,1) > [trials(i).fixAcq]+fixPad,trials(i).eye(:,1) < [trials(i).targOn]);
...
%             errants = [errants; i];
        ...
        if find(fixIdx2,1,'last') > startIdx+10;
            plotit = 1;
            trials(i).saccadeStart = NaN;
            trials(i).saccadeEnd = NaN;
            trials(i).errantSaccade = NaN;
