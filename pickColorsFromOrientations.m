function colors = pickColorsFromOrientations(orientations,luminance)
% colors = pickColorsFromOrientations(orientations)
%   grab some perceptually uniform and perceptually isoluminant colors
% colors = pickColorsFromOrientations(orientations, luminance)
%   allows you to specify the luminance you're looking for 0 to 100

nColorSamples = 1000; % for creating the distribution

if nargin == 1
    lum = 70; % range = 0 to 100
else
    lum = luminance;
end
...
colorStart = pi; %rand * 2*pi; % randomize the start?
...
% can resample this distribution in a more perceptually uniform way:
...
orientations = orientations.*(pi/180); % convert to deg
edges = orientations ./ (2*pi);%[0:((2*pi)/(nColors))/(2*pi):((2*pi)/(nColors))/(2*pi)*(nColors-1)];
...
colors = NaN(length(orientations),3);
for i = 1:length(orientations)
    colors(i,:) = [R(i),G(i),B(i)];
    
    [x,y] = pol2cart(orientations(i),ecc);
