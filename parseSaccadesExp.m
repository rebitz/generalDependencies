...
% try,        
    ...
    if trials(i).correct == 1 && sum(~isnan(trials(i).eye(:,1)))>10
        ...
        fixIdx = and(trials(i).eye(:,1) > [trials(i).fixAcq]+fixPad,trials(i).eye(:,1) < [trials(i).targAcq]-acqPad);%-[trials(i).targGap]);
...
%         %                     % PLOTZ
%                     figure(); hold on;
%                     plot(fixIdx2,'--g'); plot(xdata,'-k')
%                     plot(fixIdx,'--k'); plot(ydata,'-r')
%                     drawnow;
...
%             errants = [errants; i];
        ...
        % if they broke fixation too early
