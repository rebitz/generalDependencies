...
% from the actual experimental code!
eyeOffBy = 0.1;
...
% try,        
    % first, fix the offset in eye position time stamps:
%     % reload the timestamps:
%     t = trials(i).eyedata(1,:);
%     offset = trials(i).trackerOffset; % get the offset we queried
%     % then append these to our working eye stuff
%     t = (t/1000)+(offset-eyeOffBy); % adjust for bug, convert to ms
%     trials(i).eye(:,1) = t;
    
    trials(i).eye(:,1) = trials(i).eye(:,1) + eyeOffBy;
