function [fH,th2] = twoMixPlotCumSum(times,xMax)
...
bar(xbins+.5,cumsum(y),'FaceColor',[.5 .5 .5],'LineStyle','none',...
    'BarWidth',1);

% fZ1, shift the plotting over, but eval starting at 0
h = plot(xbins+.5,cumsum(f2(xbins,theta))./sum(f2(xbins,theta)),'-','LineWidth',3); hold on; 
...
h(2) = plot(xbins+.5,cumsum(f1(xbins,theta))./sum(f1(xbins,theta)),'--k','LineWidth',3);

% th = plot(x+.5,cumsum(th2(3)*f1(x,th2(1)))./sum(th2(3)*f1(x,th2(1))),'--');
% th(2) = plot(x+.5,cumsum((1-th2(3))*f1(x,th2(2)))./sum((1-th2(3))*f1(x,th2(2))),'--');
