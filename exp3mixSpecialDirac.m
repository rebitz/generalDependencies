function [theta,Zlabels,lik] = exp3mixSpecialDirac(times,rewards,theta)
% fit a mixture of 3 exponentials
%  theta is fitted parameters:
%   theta(1) hazard of fixed distribution
%   theta(2) hazard of rwd == 1
%   theta(3) mixing proportion
%  also a dirac at 1 conditioned on no first reward
...
D = @(x,rwd,tHat) (((tHat(1)/(1+tHat(1))).^x)*(tHat(3)/(1+tHat(1)))) + ...
    (rwd .* (((tHat(2)/(1+tHat(2))).^x)*((1-tHat(3))/(1+tHat(2))))) + ...
    ((1-rwd) .* (1-tHat(3)).*(x==0));

fZ1 = @(x,rwd,tHat) (((tHat(1)/(1+tHat(1))).^x)*(tHat(3)/(1+tHat(1)))) ./ D(x,rwd,tHat);
fZ2 = @(x,rwd,tHat) (rwd .* (((tHat(2)/(1+tHat(2))).^x)*((1-tHat(3))/(1+tHat(2))))) ./ D(x,rwd,tHat);
fZ3 = @(x,rwd,tHat) ((1-rwd) .* (1-tHat(3)).*(x==0)) ./ D(x,rwd,tHat);
...
f3 = @(x,rwd) ((theta(3)).*(1./(1+theta(1))).*((theta(1)./(1+theta(1))).^x)) + ...
    (rwd .* (1-theta(3)).*(1./(1+theta(2))).*((theta(2)./(1+theta(2))).^x) + ...
    ((1-rwd) .* (1-theta(3)).*(x==0))); % p 39 mT, first eq
