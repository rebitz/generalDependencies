...
%         xdata(ydata>3000) = NaN; ydata(ydata>3000) = NaN;
        ...
        if smoothEyeHistory
            ...
            % fixIdx2 = spatial position & low velocity
...
%             veloIdx = and(ydata <= origin(2)+(fixErr*fixErrConstant), ydata >= origin(2)-(fixErr*fixErrConstant));
...
%         figure(); plot(fixNum)
        ...
        if abs(timeOffset)>nanmean(diff(tstamps)); % if we seem to be > 1 bin off
            fprintf('\n adjusting timestamps by %d',timeOffset)
            trials(i).eye(:,1) = tstamps-timeOffset;
        end
        tstamps = trials(i).eye(:,1);
        
        % end the fixation period when the targ comes on
        fixIdx2(find(trials(i).eye(:,1) > [trials(i).targOn],1,'first'):end)=deal(false);
                
        % now we can finally deal with saccades
...
%             set(gcf,'Position',[2400 400 350 300]);
            ...
            legend([a,b,e,c,d,h],'targOn','fixOn','fixAcq','sacEnd','sacStart','fixIdx')
